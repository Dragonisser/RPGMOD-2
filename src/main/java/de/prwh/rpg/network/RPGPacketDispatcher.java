package de.prwh.rpg.network;

import de.prwh.rpg.RPGMain;
import de.prwh.rpg.network.handler.AbstractMessageHandler;
import de.prwh.rpg.network.handler.bidirectional.RPGAbstractBiMessageHandler;
import de.prwh.rpg.network.handler.client.AbstractClientMessageHandler;
import de.prwh.rpg.network.handler.client.ChangeKeyStateMessageHandler;
import de.prwh.rpg.network.handler.client.HealthNBTMessageHandler;
import de.prwh.rpg.network.handler.client.ManaNBTMessageHandler;
import de.prwh.rpg.network.handler.client.RpgClassNBTMessageHandler;
import de.prwh.rpg.network.handler.client.RpgLevelNBTMessageHandler;
import de.prwh.rpg.network.handler.client.RpgRaceNBTMessageHandler;
import de.prwh.rpg.network.handler.client.StaminaNBTMessageHandler;
import de.prwh.rpg.network.handler.server.AbstractServerMessageHandler;
 ...
 *
 */
public class RPGPacketDispatcher {
	 ...
	 */
	private static final SimpleNetworkWrapper dispatcher = NetworkRegistry.INSTANCE.newSimpleChannel(RPGMain.MODID);
	...
	public static final void registerPackets() {
		...
		// Client>Server
		// registerMessage(ClassSelectionMessageHandler.class,
		// ClassSelectionMessage.class);
		// registerMessage(RaceSelectionMessageHandler.class,
		// RaceSelectionMessage.class);
		// registerMessage(RequestSynchronizationMessageHandler.class,
		// RequestSynchronizationMessage.class);

		// Server>Client
		// registerMessage(OpenGuiMessageHandler.class, OpenGuiMessage.class);
		// registerMessage(RpgPlayerModelMessageHandler.class,
		// RpgPlayerModelMessage.class);
		registerMessage(ChangeKeyStateMessageHandler.class, ChangeKeyStateMessage.class);
		registerMessage(ManaNBTMessageHandler.class, ManaNBTMessage.class);
		registerMessage(StaminaNBTMessageHandler.class, StaminaNBTMessage.class);
		registerMessage(HealthNBTMessageHandler.class, HealthNBTMessage.class);
		registerMessage(RpgClassNBTMessageHandler.class, RpgClassNBTMessage.class);
		registerMessage(RpgRaceNBTMessageHandler.class, RpgRaceNBTMessage.class);
		registerMessage(RpgLevelNBTMessageHandler.class, RpgLevelNBTMessage.class);
	...
	private static final <REQ extends IMessage, REPLY extends IMessage> void registerMessage(
			Class<? extends IMessageHandler<REQ, REPLY>> handlerClass, Class<REQ> messageClass, Side side) {
		RPGPacketDispatcher.dispatcher.registerMessage(handlerClass, messageClass, packetId++, side);
	}

	/**
	 * Registers a message and message handler on both sides; used mainly for
	 * standard IMessage + IMessageHandler implementations and ideal for messages
	 * that are handled identically on either side
	 */
	private static final <REQ extends IMessage, REPLY extends IMessage> void registerBiMessage(
			Class<? extends IMessageHandler<REQ, REPLY>> handlerClass, Class<REQ> messageClass) {
		RPGPacketDispatcher.dispatcher.registerMessage(handlerClass, messageClass, packetId, Side.CLIENT);
		RPGPacketDispatcher.dispatcher.registerMessage(handlerClass, messageClass, packetId++, Side.SERVER);
	}

	/**
	 * Registers a message and message handler, automatically determining Side(s)
	 * based on the handler class
	 * 
	 * @param handlerClass
	 ...
	 */
	private static final <REQ extends IMessage> void registerMessage(
			Class<? extends AbstractMessageHandler<REQ>> handlerClass, Class<REQ> messageClass) {
		...
		} else if (RPGAbstractBiMessageHandler.class.isAssignableFrom(handlerClass)) {
	...
	public static final void sendTo(IMessage message, EntityPlayerMP player) {
		RPGPacketDispatcher.dispatcher.sendTo(message, player);
	}
	...
	public static final void sendToAllAround(IMessage message, NetworkRegistry.TargetPoint point) {
		RPGPacketDispatcher.dispatcher.sendToAllAround(message, point);
	}
	...
	public static final void sendToAllAround(IMessage message, int dimension, double x, double y, double z,
			double range) {
		RPGPacketDispatcher.sendToAllAround(message, new NetworkRegistry.TargetPoint(dimension, x, y, z, range));
	}
	...
	public static final void sendToAllAround(IMessage message, EntityPlayer player, double range) {
		RPGPacketDispatcher.sendToAllAround(message, player.world.provider.getDimension(), player.posX, player.posY,
				player.posZ, range);
	}
	...
	public static final void sendToDimension(IMessage message, int dimensionId) {
		RPGPacketDispatcher.dispatcher.sendToDimension(message, dimensionId);
	}
	...
	public static final void sendToServer(IMessage message) {
		RPGPacketDispatcher.dispatcher.sendToServer(message);
	}
}